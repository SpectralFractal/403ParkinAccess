...
import style from "./VideoClassic.module.css";
...
const VideoComponent = () => {
  ...
  const handleConnect = () => {
    wsRef.current = new WebSocket(googleApi);
  ...
  return (
    <div className={style.wrapper}>
      ...
      <video
        ...
        autoPlay
        playsInline
        style={{ display: showSendingVideo ? "block" : "none" }}
      />
      ...
      <div className={style.info}>
        {wsRef.current ? (
          <button onClick={closeConnection} className={style.confirmBtn}>
            {" "}
            Close{" "}
          </button>
        ) : (
          <button className={style.confirmBtn} onClick={handleConnect}>
            Conecteaza
          </button>
        )}
      </div>
